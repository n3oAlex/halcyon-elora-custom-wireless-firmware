...
extern struct ec11c_data ec11c_driver;

#include <zephyr/logging/log.h>
LOG_MODULE_DECLARE(EC11C, CONFIG_SENSOR_LOG_LEVEL);

static inline void setup_int(const struct device *dev, bool enable) {
    const struct ec11c_config *cfg = dev->config;
...
static void ec11c_a_gpio_callback(const struct device *dev, struct gpio_callback *cb,
                                 uint32_t pins) {
    struct ec11c_data *drv_data = CONTAINER_OF(cb, struct ec11c_data, a_gpio_cb);
...
static void ec11c_b_gpio_callback(const struct device *dev, struct gpio_callback *cb,
                                 uint32_t pins) {
    struct ec11c_data *drv_data = CONTAINER_OF(cb, struct ec11c_data, b_gpio_cb);
...
static void ec11c_thread_cb(const struct device *dev) {
...
int ec11c_init_interrupt(const struct device *dev) {
    struct ec11c_data *drv_data = dev->data;
    ...
    gpio_init_callback(&drv_data->a_gpio_cb, ec11c_a_gpio_callback, BIT(drv_cfg->a.pin));
    ...
    gpio_init_callback(&drv_data->b_gpio_cb, ec11c_b_gpio_callback, BIT(drv_cfg->b.pin));
